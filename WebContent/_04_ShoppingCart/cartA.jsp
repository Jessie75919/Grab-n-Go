...
<jsp:useBean id="bindRest" class="_01_Store_register.model.StoreBeanDAO"
...
<body id="top" class="cbp-spmenu-push">
	...
	<main>
	<div class="brcame">
		<a href="../indexA.jsp">首頁</a> / 購物車
	</div>
	<div id="validMsg">
		您好像還沒有認證，請盡快認證才能繼續訂餐喔 ~<br> 或是請至您的"個人資料"進行認證<br> <a
			id="validBth"
			href="../sendValidMail.do?mode=1&user=${LoginOK.memberId}">點我認證</a> <a
			href="#" onclick="closePanel()">下次再說</a>
	</div>
	<section class="content">
	<form id="theForm" action="PayBill.do" method="post"
		class="formcontent" onsubmit="return validCheck(event);">
		<div id="cartLeft" class="cartLeft wow fadeInUp">
			<h3>
				<a
					href="${pageContext.servletContext.contextPath}/_07_storePage/getOneRest.do?id=${bindRest.rest_id}">${ordRB.rest_name}&nbsp;
					<c:if test="${ordRB.rest_branch!='null'}">${ordRB.rest_branch}</c:if>
				</a>
			</h3>
			<!--菜單列表-->
			...
			<c:forEach var="list" items="${map.listAllMap}">
				<div id="List${list.prod_id}" class="foodList">
					<%--${list.prod_id}<br> --%>
					<figure> <img
						src="${pageContext.servletContext.contextPath}/_00_init/getImage?id=${list.prod_id}' alt="
						${list.item_name}" title="${list.item_name}"> </figure>
					<div class="foodInfo" name="prod_${list.prod_id}${list.item_note}">
						...
						<p>
							數量 <input type="number" class="count"
								id="count${list.prod_id}${list.item_note}"
								value="${list.item_amount}"
								onchange="modifyAmount(${list.prod_id}, '${list.item_note}')"
								min="1"> <span class="price">NT$${list.item_price}</span>
						</p>
						<textarea class="note" id="note${list.prod_id}${list.item_note}"
							placeholder="特殊需求"
							onchange="modifyNote(this,${list.prod_id}, '${list.item_note}')">${list.item_note}</textarea>
					</div>
					<div class="closeBtn">
						<i class="icon-close" title="刪除"
							onClick="deleteF(${list.prod_id}, '${list.item_note}')"></i>
					</div>
				</div>
			</c:forEach>
		...
		<div class="cartRight wow fadeInDown">
			...
			<div class="formList">
				<div class="formTitle">
					<i class="icon-user"></i>訂購人
				</div>
				<div class="formInfo">
					<input type="text" name="name" id="name" value="${LoginOK.name}"
						class="validate[required] text-input">
				</div>
			</div>
			<div class="formList">
				<div class="formTitle">
					<i class="icon-phone"></i>聯絡電話
				</div>
				<div class="formInfo">
					<input type="text" name="tel" id="tel" value="${LoginOK.phone}"
			...
			<div class="formList">
				<div class="formTitle">
					<i class="icon-birth"></i>預計取餐時間
				</div>
				<div class="formInfo">
					<c:set var="pickTime"
						value="<%=new Date(new Date().getTime() + 60 * 20 * 1000)%>" />
					<fmt:formatDate var="timeNow" type="both" pattern="HH:mm"
						value="${pickTime}" />
					<input type="time" name="time" id="time" value="${timeNow}"
						class="validate[required] text-input">
				</div>
			</div>
			<!--訂購人資訊 end-->
			<div class="formBtn">
				<p>
					<a href="../indexA.jsp">繼續逛逛</a> <input name="sub" type="submit"
						id="sub" value="送出訂單">
				</p>
			</div>
		</div>
	...
	<!--SmoothScroll js-->
	<script type="text/javascript"
		src="../javascript/SmoothScroll_v1.2.1.js"></script>
	<!--share js-->
